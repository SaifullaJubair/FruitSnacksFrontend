...
// readable layout: header band, sticky policy-nav + on-this-page TOC sidebar,
// a prose column, and a support CTA.
export default function PolicyPageLayout({ slug }) {
  ...
  // Same contract as FloatingWhatsApp: a toggle plus a raw number, not a URL.
  const waNumber = site?.enable_whatsapp_chat
    ? String(site?.whatsapp_number || "").replace(/\D/g, "")
    : "";
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-primary-900 via-primary-800 to-primary-900 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-12">
          ...
          <div className="mt-6 h-px w-24 bg-primary-400/70" />
        </div>
      </div>
            ...
            {waNumber && (
              <div className="mt-8 rounded-2xl bg-primary-900 text-white px-6 py-6 sm:px-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-[#25D366] hover:brightness-95 text-white font-semibold px-5 py-2.5 text-sm transition-all shrink-0"
                >
