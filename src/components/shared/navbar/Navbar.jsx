        ...
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100 bg-gray-50">
          {siteData?.logo && (
            <Link href="/" onClick={onClose}>
              {/* next/image so the multi-MB source is resized/WebP'd to the ~36px
        ...
        <Contain>
          <div className="flex items-center h-[72px] gap-2 lg:gap-4">
            ...
            <Link href="/" className="shrink-0 mr-1">
              {siteData?.logo ? (
                /* Was a raw <img> pulling the full-size original to paint ~49x40
                   — it competed with the LCP banner for bandwidth. next/image
                   serves a right-sized WebP; priority because it is above the
                   fold on every route. */
                <Image
                  src={siteData.logo}
                ...
                <span className="text-lg font-bold text-primary">{siteData?.title || "FruitSnacks"}</span>
              )}
            </Link>
